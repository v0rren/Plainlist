    ...
    <div className="rounded-lg border border-line bg-surface p-4">
      <div className="mb-2 flex items-baseline gap-2 text-sm">
        <span className="font-medium">
          {total ? `~${formatEstimate(total)} stimati` : 'Nessuna stima'}
        </span>
        <span className="text-muted">su {formatEstimate(capacityMin)} disponibili</span>
        {doneMin > 0 && <span className="text-success">· {formatEstimate(doneMin)} già fatti</span>}
        {over > 0 && (
          <span className="ml-auto inline-flex items-center gap-1 text-warning">
            <AlertTriangle size={14} /> oltre di {formatEstimate(over)}
          </span>
        )}
      </div>
      ...
      {unestimated > 0 && (
        <p className="mt-2 text-xs text-subtle">
          {unestimated} {unestimated === 1 ? 'task senza stima' : 'task senza stima'}: aggiungi ~30m o ~1h per un conto più preciso.
        </p>
      )}
    </div>
  )
  ...
  return (
    ...
    >
      ...
      <button
        ...
        title="Aggiungi a Il mio giorno"
        onClick={(e) => {
          e.stopPropagation()
    ...
    <div className="max-w-5xl px-6 py-5">
      <div className="mb-4 flex items-baseline gap-3">
        <h1 className="flex items-center gap-2 text-xl font-semibold">
          <Sun size={20} className="text-warning" /> Il mio giorno
        </h1>
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_340px]">
          ...
          <section>
            {day.planned.length === 0 ? (
              <div className="rounded-lg border border-dashed border-line-strong px-4 py-8 text-center text-sm text-muted">
                ...
                <br />
                Aggiungili dai suggerimenti, con l'icona <Sun size={13} className="inline" /> su ogni task o trascinandoli su
                "Il mio giorno".
              </div>
            ) : (
          ...
          {day.done.length > 0 && (
            <section>
              <h2 className="mb-1 px-2 text-xs font-semibold tracking-wide text-success uppercase">
                Fatti oggi <span className="font-normal text-subtle">{day.done.length}</span>
              </h2>
              {day.done.map((t) => (
        ...
        <aside className="h-fit space-y-3 rounded-lg border border-line bg-surface p-3">
          <h2 className="flex items-center gap-2 px-2 text-sm font-semibold">
            <Sparkles size={15} className="text-accent" /> Suggerimenti
          </h2>
          {hasSuggestions ? (
            <>
              <SuggestionGroup icon={AlertTriangle} title="Scaduti" tasks={suggestions.overdue} today={today} tone="text-danger" addAll />
              <SuggestionGroup icon={CalendarCheck} title="In scadenza oggi" tasks={suggestions.today} today={today} tone="text-accent" addAll />
              <SuggestionGroup icon={Sunrise} title="Domani" tasks={suggestions.tomorrow} today={today} />
              <SuggestionGroup icon={Flag} title="Priorità alta" tasks={suggestions.important} today={today} />
            </>
          ) : (
            <p className="px-2 text-sm text-subtle">Niente di urgente da proporre.</p>
          )}
        </aside>
      </div>
    </div>
